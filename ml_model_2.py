...
bonds_df_path = 'data/clusters data/Bonds/Bonds_df.csv'
bonds_df_cluster_path = 'data/clusters data/Bonds/Bonds_df_cluster.csv'
encoders_bonds_path = 'data/clusters data/Bonds/Bonds_encoders.pkl'
...
stocks_df_path = 'data/clusters data/Stocks/Stocks_df.csv'
stocks_df_cluster_path = 'data/clusters data/Stocks/Stocks_df_cluster.csv'
encoders_stocks_path = 'data/clusters data/Stocks/encoders_Stocks.pkl'
...
# Load the CSV files
deposit_df_path = 'data/clusters data/Deposit/Deposit_df.csv'
deposit_df_cluster_path = 'data/clusters data/Deposit/Deposit_df_cluster.csv'
encoders_deposit_path = 'data/clusters data/Deposit/Deposit_encoders.pkl'
...
def convert_input(input_dict, mapping):
    ...
    for key, value in input_dict.items():
        ...
        if key in mapping:
            ...
            if key == 'islamic_compliance':
                value = value.replace('non-Islamic Product', 'non_Islamic_Product')
            converted[new_key] = value
    return converted

def get_inference(input_data):
    # Extract JSON content from POST request
    #data = request.json

    # Convert data to DataFrame
    sample = pd.DataFrame([input_data])
    series_data = sample.iloc[0]
    # Apply the inference function
    best_assets = inferance(series_data)

    #best_assets= best_assets["Best Selection Items"].sample().iloc[0]
    result = answer(best_assets)
    # Return results as JSON
    return result


input_data ={'age_group': '>65', 'geography': 'EUROPE', 'currency': 'ANY', 'risk_appetite': 'High Risk', 'investment_horizon': '3 years', 'financial_expertise': 'Expert', 'investment_sector': 'Real Estate', 'investment_products': 'Cash', 'investment_amount': '>10M', 'financial_goal': 'Grow As much as possible', 'islamic_compliance': 'non-Islamic Product'}
mapping_input_data= convert_input(input_data, mapping)

print(get_inference(mapping_input_data))
